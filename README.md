...
누구나 답변을 "추가"는 할 수 있고 "삭제"(board.html의 카드 삭제 버튼)도 가능하지만,
기존 답변의 **내용을 몰래 수정**하는 것은 막는 규칙입니다.
...
7. 명사 빈도표에서 **단어를 클릭**하면 그 아래 "원문 보기" 영역에 해당 단어가
   ...
   **하나라도 포함된** 답변이 모두 표시됩니다 (OR 조건). "선택 초기화"로
   ...
   각 단어 오른쪽의 **✕ 버튼**을 누르면 그 단어를 상위 15개 목록에서
   제외할 수 있습니다 (예: 의미 없는 단어가 순위에 끼어들 때). 제외한 단어는
   표 아래에 칩으로 나열되며, 칩을 다시 누르거나 "전체 복원"으로 되돌릴 수
   있습니다. 이 제외 목록은 화면에서만 적용되며(개인 설정), 새로고침하면
   초기화됩니다.
8. `board.html`을 **카드형**으로 전환하면 각 카드 우측 상단에 **✕ 삭제 버튼**이
   있어, 부적절하거나 잘못 등록된 답변을 개별적으로 지울 수 있습니다. 삭제하면
   Firebase에서도 실제로 지워지므로 워드클라우드·명사 빈도표에도 즉시 반영됩니다.
   (저장된 결과를 ▶ 재생 중인 카드에는 삭제 버튼이 뜨지 않습니다 — 그건 과거
   스냅샷이라 실시간 데이터와 연결되어 있지 않기 때문입니다.)
9. `board.html` 우측 상단의 **"결과 저장"** 버튼을 누르면 지금 시점의 질문과
